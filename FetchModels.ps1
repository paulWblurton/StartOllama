...
GenerateModelDashboard.ps1 — Jaguar Edition with Scrollable, Multi‑Column Navigation,
  Section Headers, and Fixed System Requirements Table
---------------------------------------------------------------------------------------
How It Works:
...
3. Optionally filters models using a user-provided filter string.
4. Builds an HTML dashboard that includes:
   • A top navigation index wrapped in a scrollable container with a fixed max-height (so it shows about 5 rows) and multi‑column layout.
   • A system requirements table with fixed, cleanly encoded text.
   • Detailed sections for each model (each section begins with an H2 header) that display the title, description, (optional) classification, default command link, any variants, and a “Back to top” link.
